...
for (const file of ['index.html', 'styles.css', 'anki-desktop.css', 'pengram-theme.css', 'pengram-overrides.css']) await cp(path.join(root, file), path.join(dist, file));
...
const precache = ['./', './index.html', './styles.css', './anki-desktop.css', './pengram-theme.css', './pengram-overrides.css', './manifest.webmanifest', './icon.svg', ...vendor.map((file) => `./vendor/${file}`), ...assets.map((file) => `./assets/${file}`)];

...
for (const file of ['index.html', 'styles.css', 'anki-desktop.css', 'pengram-theme.css', 'pengram-overrides.css', 'sync-sandbox.html', 'sync-sandbox.js']) await cp(path.join(root, file), path.join(dist, file));
...
const precache = ['./', './index.html', './styles.css', './anki-desktop.css', './pengram-theme.css', './pengram-overrides.css', './sync-sandbox.html', './sync-sandbox.js', './manifest.webmanifest', './icon.svg', ...vendor.map((file) => `./vendor/${file}`), ...assets.map((file) => `./assets/${file}`)];

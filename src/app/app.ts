...
export class App {
  ...
  private async renderSettings(): Promise<void> {
    ...
    for (const [key, labelText, checked] of toggles) { const label = el('label', { className: 'check-row', text: labelText }), input = el('input', { attrs: { type: 'checkbox' } }); input.checked = checked; input.addEventListener('change', () => void saveSettings({ ...settings, [key]: input.checked })); label.prepend(input); review.append(label); }
    grid.append(review);
